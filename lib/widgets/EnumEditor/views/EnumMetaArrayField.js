...
class EnumMetaArrayField extends React.Component {
        ...
        return (
            <div className={bem(bem.block(), 'form-inline')}>
                <div className='pull-right text-muted'>
                    <small>Используйте <span className='label label-default'>Shift</span> + <span
                        className='label label-default'>↑↓</span> для перехода по полям
                    </small>
                </div>
                <h3>
    ...
    _onAddColumn() {
        ...
        meta.forEach((item, i) => {
            this.props.dispatch(change(EnumMetaArrayField.formId, `meta.${i}.customColumns.${columnName}`, null));
        });
    }
    ...
    _onRemoveColumn(columnName) {
        ...
        const meta = this.props.formValues.meta || {};
        meta.forEach((item, i) => {
            this.props.dispatch(change(EnumMetaArrayField.formId, `meta.${i}.customColumns.${columnName}`, ''));
        });
    }
